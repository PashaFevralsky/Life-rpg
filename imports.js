...
/* Life RPG 8.0.3 — Bank, OCR and import engine */
...
function importHistoricalCutoff(accountId=defaultAccountId()){const a=(S.accounts||[]).find(x=>x.id===accountId);return a?.verifiedAt||(accountId===defaultAccountId()?S.settings.cashBalanceVerifiedAt:"")||""}

function importIsHistorical(dateKey,occurredAt="",accountId=defaultAccountId()){
  const cutoff=importHistoricalCutoff(accountId);if(!cutoff)return false;const cutoffTs=Date.parse(cutoff);if(!Number.isFinite(cutoffTs))return false;const cutoffDate=localDateKey(new Date(cutoffTs));
...
function adjustImportCompensationOnRestore(x){if(!x?.importBatchId||!x.importCashContribution)return;const a=(S.cashAdjustments||[]).find(z=>z.importBatchId===x.importBatchId);if(a)a.delta=(+a.delta||0)-(+x.importCashContribution||0)}
...
async function restoreLastDeleted(){const z=S.trash?.[0];if(!z){toast("Корзина пуста");return}const x=deepClone(z.item),exists=(arr,id)=>arr.some(v=>v.id===id);if(z.kind==="income"){if(!exists(S.incomeLogs,x.id)){S.incomeLogs.unshift(x);adjustImportCompensationOnRestore(x)}}else if(z.kind==="expense"){if(!exists(S.expenses,x.id)){reapplyReservationUse(x.reservationUse);S.expenses.unshift(x);adjustImportCompensationOnRestore(x)}}else if(z.kind==="transfer"){if(!exists(S.bankTransfers,x.id))S.bankTransfers.unshift(x)}else if(z.kind==="asset_transfer"){if(!exists(S.assetTransfers,x.id))S.assetTransfers.unshift(x)}else if(z.kind==="work"){if(!exists(S.workLogs,x.id)){if(x.sourceDealId&&(!S.crmDeals.some(d=>d.id===x.sourceDealId)||S.workLogs.some(w=>w.sourceDealId===x.sourceDealId))){toast("Связанная сделка отсутствует или её реализация уже учтена");return}x.xpAward=Math.min(Math.max(0,+x.xpAward||0),Math.max(0,120-workXpOnDate(x.date)));S.workLogs.unshift(x);if(x.sourceDealId){const deal=S.crmDeals.find(d=>d.id===x.sourceDealId);deal.realizedAmount=x.sales;deal.realizationDate=x.date;deal.stage="Выиграно";deal.probability=100;}addXp(x.xpAward||0,"Карьера","Восстановлена рабочая запись",`work:${x.id}`,"process",x.date)}}else if(z.kind==="tennis"){if(!exists(S.tennis,x.id)){S.tennis.unshift(x);addXp(x.xpAward||0,"Теннис","Восстановлена тренировка",`tennis:${x.id}`,"process",x.dateKey);recomputeTennisElo()}}else if(z.kind==="reading"){if(!exists(S.readingLogs,x.id)){if(!S.books.some(b=>b.id===x.bookId)){toast("Сначала восстанови книгу этой сессии");return}S.readingLogs.unshift(x);addXp(x.xpAward||0,"Разум","Восстановлено чтение",`read:${x.id}`,x.completionBonus?"result":"process",x.dateKey);recomputeBookProgress(x.bookId);reconcileReadingAwards();syncAutoDailyQuests(x.dateKey)}}else if(z.kind==="crm"){if(!exists(S.crmDeals,x.id))S.crmDeals.unshift(x)}else if(z.kind==="book"){if(!exists(S.books,x.id))S.books.push(x)}else{toast("Этот тип нельзя восстановить автоматически");return}S.trash.shift();audit("Восстановлено",z.kind,x.note||x.source||x.name||x.title||"");await save("Последняя удалённая запись восстановлена") }
...
function buildAiContext(){const wf=typeof crmForecastData==="function"?crmForecastData():null,ts=typeof computeTennisElo==="function"?computeTennisElo():null,ls=typeof lifeScore==="function"?lifeScore():null,book=typeof currentBook==="function"?currentBook():null;return {format:"life-rpg-context-v1",appVersion:APP_VERSION,stateVersion:S.version,generatedAt:new Date().toISOString(),forecastSettings:{primaryAccount:accountName(defaultAccountId()),dailySpendLimit:+S.settings.dailySpendLimit||0,minimumCashFloor:+S.settings.minimumCashFloor||0,monthlyDebtGoal:+S.settings.monthlyDebtGoal||0,emergencyFundBalance:+S.settings.emergencyFundBalance||0,liquidityTargetDays:+S.settings.liquidityTargetDays||14,envelopeLimits:{...(S.envelopeLimits||{})}},accounts:activeAccounts().map(a=>({name:a.name,type:a.type,balance:accountBalanceById(a.id),verifiedAt:a.verifiedAt||"",primary:a.id===defaultAccountId()})),assets:(S.assets||[]).filter(a=>a.active!==false).map(a=>({name:a.name,type:a.type,value:assetValueById(a.id),liquid:a.liquid,available:a.available,verifiedAt:a.verifiedAt||""})),debts:(S.debts||[]).filter(d=>d.active!==false).map(d=>({name:d.name,type:d.type,balance:d.balance,rate:d.rate,rateKnown:d.rateKnown!==false,basePayment:d.min,nextPaymentAmount:d.nextPaymentAmount||0,nextPaymentDate:d.nextPaymentDate||"",paymentMode:d.paymentMode,limit:d.limit||0,balanceVerifiedAt:d.balanceVerifiedAt||""})),incomeSchedule:(S.settings.incomeEvents||[]).map(x=>({day:x.day,label:x.label,amount:x.amount})),regularPayments:(S.regularPayments||[]).filter(x=>x.active!==false).map(x=>({name:x.name,amount:x.amount,dueDay:x.dueDay,category:x.category,mandatory:x.mandatory!==false})),recentTransactions:unifiedTransactions().slice(0,80).map(x=>({date:x.dateKey,type:x.kind,title:x.title,category:x.category,account:x.account,amount:x.amount,note:x.note})),importRules:(S.importRules||[]).filter(x=>x.active!==false).map(x=>({keyword:x.keyword,type:x.type,category:x.category||""})),work:{plan:+S.settings.workMonthlyPlan||0,month:typeof workMonth==="function"?workMonth():{},forecast:wf,topActions:typeof crmDecisionEngine==="function"?crmDecisionEngine().slice(0,8):[],deals:(S.crmDeals||[]).slice(0,30).map(d=>({name:d.name,client:d.client||"",city:d.city||"",region:d.region||"",potential:+d.potential||0,stage:d.stage||"",probability:+d.probability||0,nextStep:d.nextStep||"",nextDate:d.nextDate||"",closeDate:d.closeDate||"",lpr:d.lpr||"",procurement:d.procurement||"",competitor:d.competitor||"",manufacturers:d.manufacturers||"",projectDocs:d.projectDocs||"",realizedAmount:+d.realizedAmount||0,realizationDate:d.realizationDate||""}))},tennis:{internalElo:ts?.rating??S.settings.tennisElo,officialRating:+S.settings.tennisOfficialRating||0,monthlyTarget:+S.settings.tennisMonthlyTarget||12,focusRecommendation:typeof tennisFocusRecommendation==="function"?tennisFocusRecommendation():"",load7:typeof tennisLoad7==="function"?tennisLoad7():0,recentSessions:(S.tennis||[]).slice(0,20).map(x=>({date:x.dateKey,type:x.type,minutes:x.min,focus:x.focus,load:x.load,w:x.w,l:x.l,matches:Array.isArray(x.matches)?x.matches:[],note:x.note||""}))},knowledge:{readingDailyMin:+S.settings.readingDailyMin||30,currentBook:book?{title:book.title,author:book.author||"",currentPage:book.currentPage||0,totalPages:book.totalPages||0}:null,queue:typeof readingQueueSorted==="function"?readingQueueSorted().slice(0,30).map(b=>({order:b.readingOrder,title:b.title,author:b.author||""})):[],recentNotes:(S.readingLogs||[]).filter(x=>x.note||x.application).slice(0,30).map(x=>({date:x.dateKey,book:S.books.find(b=>b.id===x.bookId)?.title||"",chapter:x.chapter||"",tags:x.tags||[],thesis:x.note||"",application:x.application||""}))},life:{score:ls?{total:ls.total,finance:ls.finance,work:ls.career,tennis:ls.tennis,knowledge:ls.reading,system:ls.discipline,details:ls.details}:null,level:typeof level==="function"?level():null,xp:S.xpEarned,availableXp:typeof availableXp==="function"?availableXp():null},diagnostics:dataIntegrityIssues(),netWorth:netWorth()}}
...
async function handleAiImportFile(file){try{let obj=JSON.parse(await file.text());const checked=window.LifePlatform?.validateAiPackage?.(obj);if(checked&&!checked.ok)throw new Error(checked.error||"Пакет не прошёл проверку");obj=checked?.data||obj;if(obj.format==="life-rpg-statement-import-v1"){if(!Array.isArray(obj.transactions)||!obj.transactions.length)throw new Error("В пакете выписки нет операций");statementImportPackage=obj;aiImportQueue=[];renderAiImportPreview();$("aiImportStatus").textContent=`Выписка готова: ${obj.transactions.length} операций`;const review=statementReviewStats(obj);$("aiApplyBtn").disabled=review.needsReview>0;$("aiApplyBtn").textContent=review.needsReview?`Проверить ${review.needsReview} и импортировать`:"Импортировать выписку";return}statementImportPackage=null;if(obj.format!=="life-rpg-ai-import-v1"||!Array.isArray(obj.actions))throw new Error("Неверный формат пакета");aiImportQueue=obj.actions.map((a,i)=>({id:a.id||`a${i+1}`,include:a.include!==false,...a}));renderAiImportPreview();$("aiImportStatus").textContent=`Загружено действий: ${aiImportQueue.length}`;$("aiApplyBtn").disabled=!aiImportQueue.length;$("aiApplyBtn").textContent="Применить подтверждённые действия"}catch(e){statementImportPackage=null;$("aiImportStatus").innerHTML=`<span class="csv-bad">${escapeHtml(e.message||String(e))}</span>`;aiImportQueue=[];renderAiImportPreview();$("aiApplyBtn").disabled=true;$("aiApplyBtn").textContent="Применить подтверждённые действия"}}
