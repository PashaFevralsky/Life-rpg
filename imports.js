...
async function restoreLastDeleted(){const z=S.trash?.[0];if(!z){toast("Корзина пуста");return}const x=deepClone(z.item);if(z.kind==="income"){S.incomeLogs.unshift(x);adjustImportCompensationOnRestore(x)}else if(z.kind==="expense"){reapplyReservationUse(x.reservationUse);S.expenses.unshift(x);adjustImportCompensationOnRestore(x)}else if(z.kind==="transfer")S.bankTransfers.unshift(x);else if(z.kind==="asset_transfer")S.assetTransfers.unshift(x);else{toast("Этот тип нельзя восстановить автоматически");return}S.trash.shift();audit("Восстановлено",z.kind,x.note||x.source||"");await save("Последняя удалённая операция восстановлена")}
