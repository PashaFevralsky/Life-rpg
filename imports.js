...
/* Life RPG 8.0 — Bank, OCR and import engine */
...
function bankSyncTolerance(){return Math.max(0,Number($("bankSyncTolerance")?.value||1)||1)}
...
async function rollbackImportBatch(id){const b=(S.importBatches||[]).find(x=>x.id===id);if(!b){toast("Пакет не найден");return};if(b.source==="statement"&&!b.preSnapshotTs){toast("Старый statement-пакет 7.0.x не имеет полного снимка до импорта. Для безопасного возврата используй резервную копию.");return}if(!confirm(`Отменить импорт из ${b.count} операций?`))return;if(b.preSnapshotTs){try{if(!db)await openDB();const snap=await dbGet("backups",b.preSnapshotTs);if(snap?.state){S=normalizeState(snap.state);await persist();render();toast("Импорт полностью отменён");return}}catch(e){console.warn(e)}}for(const it of b.items||[]){if(it.kind==="income")S.incomeLogs=S.incomeLogs.filter(x=>x.id!==it.id);else if(it.kind==="expense"){const x=S.expenses.find(z=>z.id===it.id);if(x)restoreReservationUse(x.reservationUse);S.expenses=S.expenses.filter(x=>x.id!==it.id)}else if(it.kind==="debt_payment"){const x=S.payments.find(z=>z.id===it.id);if(x&&!x.historicalOnly){const d=debtById(x.debtId);if(d)d.balance+=+x.amount||0}if(x)restoreReservationUse(x.reservationUse);S.payments=S.payments.filter(x=>x.id!==it.id)}else if(it.kind==="transfer")S.bankTransfers=S.bankTransfers.filter(x=>x.id!==it.id);else if(it.kind==="asset_transfer")S.assetTransfers=S.assetTransfers.filter(x=>x.id!==it.id);if(it.fp)S.bankImportIds=S.bankImportIds.filter(x=>x!==it.fp)}if(b.accountId&&b.accountBefore){const a=S.accounts.find(x=>x.id===b.accountId);if(a){a.verifiedBalance=b.accountBefore.verifiedBalance;a.verifiedAt=b.accountBefore.verifiedAt||""}}S.cashAdjustments=S.cashAdjustments.filter(x=>x.importBatchId!==id);S.importBatches=S.importBatches.filter(x=>x.id!==id);audit("Откат импорта","finance",`${b.count} операций`);await save("Импорт отменён")}
...
function cleanupStatementPrerequisites(pkg){const c=pkg?.cleanup||{},sources=new Set(c.removeImportedSources||[]),from=String(c.dateFrom||"");if(!sources.size)return 0;const removedIds=new Set(),removedFps=new Set();let removed=0;const hit=x=>sources.has(x?.imported)&&(!from||statementImportedDate(x)>=from),track=x=>{removed++;removedIds.add(x.id);if(x.fp)removedFps.add(x.fp)};S.incomeLogs=(S.incomeLogs||[]).filter(x=>{if(!hit(x))return true;track(x);return false});S.expenses=(S.expenses||[]).filter(x=>{if(!hit(x))return true;restoreReservationUse(x.reservationUse);track(x);return false});S.bankTransfers=(S.bankTransfers||[]).filter(x=>{if(!hit(x))return true;track(x);return false});S.assetTransfers=(S.assetTransfers||[]).filter(x=>{if(!hit(x))return true;track(x);return false});S.payments=(S.payments||[]).filter(x=>{if(!x?.historicalOnly||!hit(x))return true;restoreReservationUse(x.reservationUse);track(x);return false});if(removedFps.size)S.bankImportIds=(S.bankImportIds||[]).filter(x=>!removedFps.has(x));const droppedBatches=new Set();S.importBatches=(S.importBatches||[]).map(b=>{const items=(b.items||[]).filter(i=>!removedIds.has(i.id));if(!items.length){droppedBatches.add(b.id);return null}return {...b,items,count:items.length}}).filter(Boolean);if(droppedBatches.size)S.cashAdjustments=(S.cashAdjustments||[]).filter(x=>!droppedBatches.has(x.importBatchId));return removed}
