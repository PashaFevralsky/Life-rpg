...
assert.ok(runtimeFiles.length>=38,`Runtime registry unexpectedly small: ${runtimeFiles.length}`);
assert.equal(new Set(runtimeFiles).size,runtimeFiles.length,"Duplicate runtime files in bootstrap registry");
const requiredRuntime=["work-growth.js","tennis-decision.js","knowledge-decision.js","today-execution.js","integration-12.5.js","feedback-os.js","import-hub.js"];
...
const sw=read("sw.js"),manifest=read("manifest.webmanifest"),core=read("core.js"),vite=read("vite.config.mjs"),pwa=read("pwa.js"),state=read("state.js"),integration=read("integration-12.5.js"),hub=read("import-hub.js");
...
assert.ok(bootstrap.includes('["ensureIntegration125Ui","renderIntegration125"]'));assert.ok(bootstrap.includes('["ensureFeedback126Ui","renderFeedback126"]'));assert.ok(bootstrap.includes('["ensureImport127Ui","renderImport127"]'));
...
assert.ok(hub.includes("createPreActionSnapshot")&&hub.includes("rollbackImportBatch")&&hub.includes("personalImportPreviewRows"));
...
assert.ok(fs.existsSync(path.join(root,"reading-list.test.js")));assert.ok(fs.existsSync(path.join(root,"tennis-huawei.test.js")));assert.ok(fs.existsSync(path.join(root,"layout-all.e2e.test.js")));assert.ok(fs.existsSync(path.join(root,"import-hub-12.7.test.js")));
