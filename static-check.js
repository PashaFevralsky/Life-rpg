...
const osModules=['data-os','projects-os','goals-os','review-os','calendar-os','tasks-os','routines-os','inbox-os','rules-os','insights-os','command-os','execution-os','decision-os','recovery-os','life-os'];
...
assert.ok(sw.includes('life-rpg-v11.0.0-core-intelligence'),'Wrong SW cache');
assert.ok(!sw.includes('?v=10.0.2')&&!sw.includes('?v=10.1.0')&&!sw.includes('?v=10.2.0'),'Stale SW asset query versions');
for(const m of osModules)assert.ok(sw.includes(`./${m}.js?v=11.0.0`),`SW missing ${m}.js`);
assert.ok(manifest.includes('Life RPG 11.0.0'),'Wrong manifest version');
assert.ok(core.includes('APP_VERSION="11.0.0"'),'Wrong app version');
...
assert.ok(bootstrap.includes('LIFE_RPG_110_MODULES'),'Modular bootstrap registry missing');
...
assert.ok(html.includes('window.__LIFE_RPG_HTML_VERSION__="11.0.0"'),'HTML shell version mismatch');
assert.ok(html.includes('<title>Life RPG 11.0.0</title>'),'HTML title mismatch');
assert.ok(!html.includes('10.2.0')&&!html.includes('10.1.0'),'Stale release references remain in HTML shell');
