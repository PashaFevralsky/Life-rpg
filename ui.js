...
/* Life RPG 8.0.1 — UI rendering and UX shell */
...
function initUi(){if(window.__LIFE_RPG_HTML_VERSION__&&window.__LIFE_RPG_HTML_VERSION__!==APP_VERSION){location.replace(`./?v=${encodeURIComponent(APP_VERSION)}&t=${Date.now()}`);return}for(const id of ["workDate","ttDate","readDate"]){if(!$(id))continue;$(id).max=localDateKey();if(!$(id).value)$(id).value=localDateKey()}if($("crmNextDate")&&!$("crmNextDate").value)$("crmNextDate").value="";
