...
/* Life RPG 8.0 — UI rendering and UX shell */
...
function renderMore(){renderBooks();renderReadingDashboard();renderSkillTree();renderRewards();renderAchievements();renderSeasonHistory();renderMonthlyLifeReport();renderIncomeScheduleEditor();renderSnapshots();renderEmergencyFund();renderPersonalAnalytics();renderWeeklyReview();renderXpBreakdown();renderKnowledgeBase();renderAudit();$("profileName").value=S.profile.name||"";$("profileGoal").value=S.profile.goal||"";$("settingWorkPlan").value=S.settings.workMonthlyPlan;$("settingIncome").value=plannedIncomeForMonth();$("settingDebtGoal").value=S.settings.monthlyDebtGoal;$("settingDailySpend").value=S.settings.dailySpendLimit;if($("settingCashFloor"))$("settingCashFloor").value=S.settings.minimumCashFloor||0;if($("settingLiquidityDays"))$("settingLiquidityDays").value=S.settings.liquidityTargetDays||14;if($("settingTennisTarget"))$("settingTennisTarget").value=S.settings.tennisMonthlyTarget||12;if($("settingReadingMin"))$("settingReadingMin").value=S.settings.readingDailyMin||30;if($("settingTennisBaseElo"))$("settingTennisBaseElo").value=S.settings.tennisBaseElo||1000;for(const [k,id,f] of [["contacts","settingWorkContacts",20],["followups","settingWorkFollowups",10],["lpr","settingWorkLpr",3],["meetings","settingWorkMeetings",3],["proposals","settingWorkProposals",3]])if($(id))$(id).value=workTarget(k,f);if($("learnImportRules"))$("learnImportRules").checked=S.settings.learnImportRules!==false;if($("autoReserveAfterImport"))$("autoReserveAfterImport").checked=S.settings.autoReserveAfterImport!==false;$("notificationStatus").textContent=`Разрешение: ${("Notification" in window)?Notification.permission:"не поддерживается"}`;$("versionStatus").textContent=`Life RPG ${APP_VERSION} • схема данных v${S.version} • обновлено ${new Date(S.updated).toLocaleString("ru-RU")}`}

async function saveSettings(){S.profile.name=$("profileName").value.trim()||"Павел";S.profile.goal=$("profileGoal").value.trim();S.settings.workMonthlyPlan=Math.max(0,+$("settingWorkPlan").value||0);S.settings.monthlyIncome=plannedIncomeForMonth();S.settings.monthlyDebtGoal=Math.max(0,+$("settingDebtGoal").value||0);S.settings.dailySpendLimit=Math.max(0,+$("settingDailySpend").value||0);S.settings.minimumCashFloor=Math.max(0,+$("settingCashFloor")?.value||0);S.settings.liquidityTargetDays=clamp(Math.round(+$("settingLiquidityDays")?.value||14),1,180);S.settings.tennisMonthlyTarget=Math.max(1,+$("settingTennisTarget")?.value||12);S.settings.readingDailyMin=Math.max(1,+$("settingReadingMin")?.value||30);S.settings.tennisBaseElo=Math.max(0,+$("settingTennisBaseElo")?.value||1000);for(const [k,id,f] of [["contacts","settingWorkContacts",20],["followups","settingWorkFollowups",10],["lpr","settingWorkLpr",3],["meetings","settingWorkMeetings",3],["proposals","settingWorkProposals",3]])S.workTargets[k]=Math.max(0,+$(id)?.value||f);recomputeTennisElo();audit("Настройки сохранены","settings","");await save("Настройки сохранены")}

function initUi(){if(window.__LIFE_RPG_HTML_VERSION__&&window.__LIFE_RPG_HTML_VERSION__!==APP_VERSION){location.replace(`./?v=${encodeURIComponent(APP_VERSION)}&t=${Date.now()}`);return}for(const id of ["crmNextDate","workDate","ttDate","readDate"])if($(id)&&!$(id).value)$(id).value=localDateKey();
...
const UX7_META={
  ...
  finance:{title:"Деньги",desc:()=>"Что есть → что делать → почему",tabs:[["overview","Сейчас"],["operations","Операции"],["debts","Долги"],["analysis","Прогноз"],["more","Ещё"]]},
...
function ux7OpenInbox(triggerFile=false){ux7Go("finance","overview");if(triggerFile)setTimeout(()=>$("smartInboxInput")?.click(),300)}
