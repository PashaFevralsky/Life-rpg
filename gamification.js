...
function renderLifeBosses(){const box=$("lifeBosses");if(!box)return;const debt=debtPct(),plan=+S.settings.workMonthlyPlan||0,sales=plan>0?clamp(workMonth().sales/plan*100,0,100):0,tt=clamp(S.tennis.filter(x=>x.dateKey?.startsWith(localMonthKey())).length/Math.max(1,S.settings.tennisMonthlyTarget||12)*100,0,100),b=currentBook(),next=nextQueuedBook(),book=b&&+b.totalPages>0?clamp((+b.currentPage||0)/(+b.totalPages||1)*100,0,100):0,bookMeta=b?`${+b.currentPage||0}/${+b.totalPages||0} стр.`:next?`следующая: ${next.title}`:"нет книг в очереди";box.innerHTML=[["Долги",debt,`${rub(totalDebt())} осталось`],["Продажи",sales,plan>0?`${rub(workMonth().sales)} / ${rub(plan)}`:"план не задан"],["Теннис",tt,`${S.tennis.filter(x=>x.dateKey?.startsWith(localMonthKey())).length}/${S.settings.tennisMonthlyTarget||12} сесс.`],["Книга",book,bookMeta]].map(x=>`<div class="boss-mini"><div class="goal-top"><b>${x[0]}</b><span>${Math.round(x[1])}%</span></div><div class="progress"><i style="width:${x[1]}%"></i></div><div class="sub">${escapeHtml(x[2])}</div></div>`).join("")}
