...
/* Life RPG 12.0.0 — modular runtime bootstrap + Growth OS + Personal OS */
const LIFE_RPG_111_MODULES=[
  ...
  "tracking-os.js","personal-os.js","journal-os.js","people-os.js","focus-os.js","body-os.js","home-os.js","capture2-os.js","personal-import-os.js","dashboard-os.js","personal-stabilization-os.js",
  "knowledge-growth.js","tennis-growth.js","rpg-growth.js","life-os.js","personal-integration-os.js"
];
function life111LoadScript(file){return new Promise((resolve,reject)=>{const existing=document.querySelector?.(`script[data-life111="${file}"]`);if(existing){if(existing.dataset.ready==="1")resolve();else existing.addEventListener("load",resolve,{once:true});return}const s=document.createElement("script");s.src=`./${file}?v=${APP_VERSION}`;s.async=false;s.dataset.life111=file;s.onload=()=>{s.dataset.ready="1";resolve()};s.onerror=()=>reject(new Error(`Не удалось загрузить ${file}`));document.head.appendChild(s)})}
function life120RefreshReleaseLabels(){
...
function life111InstallRuntime(){
  ...
  render=function(){
    ...
    ensureTrackingOsUi();renderTrackingOs();
    ensureJournalOsUi();renderJournalOs();ensurePeopleOsUi();renderPeopleOs();ensureFocusOsUi();renderFocusOs();ensureBodyOsUi();renderBodyOs();ensureHomeOsUi();renderHomeOs();ensureCapture2Ui();renderCapture2();
    ensureInboxOsUi();renderInboxOs();ensureExecutionOsUi();renderExecutionOs();ensureCalibrationOsUi();renderCalibrationOs();ensureDecisionOsUi();renderDecisionOs();ensureRulesOsUi();renderRulesOs();ensureInsightsOsUi();renderInsightsOs();ensureCommandOsUi();renderCommandOs();ensureDataOsUi();renderDataOs();ensureRecoveryOsUi();renderRecoveryOs();
    ensureKnowledgeGrowthUi();renderKnowledgeGrowth();ensureTennisGrowthUi();renderTennisGrowth();ensureRpgGrowthUi();renderRpgGrowth();
    ensurePersonalImportUi();renderPersonalImportUi();ensureDashboardOsUi();renderDashboardOs();
    requestAnimationFrame(()=>{life120RefreshReleaseLabels();renderUx7FinancePulse();renderUx7TodayPulse();ux7RefreshHeaders();for(const id of Object.keys(UX7_META))ux7SetView(id,UX7_PREFS[id]||UX7_DEFAULTS[id],false);window.LifePlatform?.refreshIcons?.();if(typeof dashboardApply==="function")dashboardApply()})
  };
...
async function life111Boot(){life120RefreshReleaseLabels();if(!window.__LIFE_RPG_MODULES_PRELOADED__)for(const file of LIFE_RPG_111_MODULES)await life111LoadScript(file);life111InstallRuntime();ux7InstallShell();initUi();await loadState();if(typeof calibrationOnBoot==="function")await calibrationOnBoot()}
