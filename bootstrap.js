...
/* Life RPG 11.1.1 — modular runtime bootstrap + Growth OS + Personal OS */
...
function life111InstallRuntime(){
  ...
  render=function(){
    ...
    requestAnimationFrame(()=>{renderUx7FinancePulse();renderUx7TodayPulse();ux7RefreshHeaders();for(const id of Object.keys(UX7_META))ux7SetView(id,UX7_PREFS[id]||UX7_DEFAULTS[id],false);window.LifePlatform?.refreshIcons?.();if(typeof dashboardApply==="function")dashboardApply()})
  };
...
async function life111Boot(){window.__LIFE_RPG_HTML_VERSION__=APP_VERSION;document.title=`Life RPG ${APP_VERSION}`;if(!window.__LIFE_RPG_MODULES_PRELOADED__)for(const file of LIFE_RPG_111_MODULES)await life111LoadScript(file);life111InstallRuntime();ux7InstallShell();initUi();await loadState();if(typeof calibrationOnBoot==="function")await calibrationOnBoot()}
