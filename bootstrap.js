...
async function lifeBoot(){lifeRefreshReleaseLabels();if(!window.__LIFE_RPG_MODULES_PRELOADED__)for(const file of LIFE_RPG_RUNTIME_MODULES)await lifeRuntimeLoadScript(file);lifeInstallRuntime();ux7InstallShell();initUi();await loadState();if(typeof calibrationOnBoot==="function")await calibrationOnBoot()}
