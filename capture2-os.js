...
function renderCapture2(){
  const box=document.getElementById("inboxOsCommand");if(!box)return;const rows=inboxOpen().slice(0,8),deals=(S.crmDeals||[]).filter(d=>!["Выиграно","Проиграно"].includes(d.stage));
  box.innerHTML=rows.length?rows.map(x=>{const s=capture2Suggest(x.text);return `<div class="log-item"><div class="split"><div><div class="qtitle">${escapeHtml(x.text)}</div><div class="qmeta">Предположение: ${capture2Label(s)} • ${escapeHtml(x.area||"Личное")}${x.dateKey?` • ${fmtDate(parseLocal(x.dateKey))}`:""}</div></div><button class="btn secondary small" onclick="capture2Route('${x.id}','${s}')">→ ${capture2Label(s)}</button></div><div class="split" style="margin-top:7px"><button class="btn ghost small" onclick="capture2Route('${x.id}','task')">→ Задача</button><button class="btn ghost small" onclick="capture2Route('${x.id}','calendar')">Событие</button><button class="btn ghost small" onclick="capture2Route('${x.id}','project')">Проект</button><button class="btn ghost small" onclick="capture2Route('${x.id}','decision')">Решение</button><button class="btn ghost small" onclick="capture2Route('${x.id}','journal')">Дневник</button><button class="btn ghost small" onclick="capture2Route('${x.id}','person')">Человек</button><button class="btn ghost small" onclick="capture2Route('${x.id}','knowledge')">Знание</button><button class="btn ghost small" onclick="capture2Route('${x.id}','tracker')">Трекер</button></div>${deals.length?`<div class="split" style="margin-top:7px"><select id="inboxCrm_${x.id}"><option value="">CRM-сделка…</option>${deals.map(d=>`<option value="${d.id}">${escapeHtml(d.name||"Сделка")}</option>`).join("")}</select><button class="btn ghost small" onclick="capture2Route('${x.id}','crm')">→ CRM</button><button class="btn ghost small" onclick="discardInbox('${x.id}')">×</button></div>`:`<button class="btn ghost small" style="margin-top:7px" onclick="discardInbox('${x.id}')">Закрыть</button>`}</div>`}).join(""):'<div class="empty">Inbox пуст.</div>'
}
