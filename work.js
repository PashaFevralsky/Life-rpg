...
function crmDecisionEngine(){const today=localDateKey(),items=[];for(const d of S.crmDeals||[]){if(["Выиграно","Проиграно"].includes(d.stage))continue;const potential=+d.potential||0,q=crmCompleteness(d),boost=Math.min(25,potential/100000);if(d.nextDate&&d.nextDate<today)items.push({score:120+boost,dealId:d.id,title:d.name,meta:`Просрочено: ${d.nextStep||"следующий шаг"} • ${rub(potential)}`,kind:"overdue"});else if(!d.nextStep||!d.nextDate)items.push({score:105+boost,dealId:d.id,title:d.name,meta:`Нет следующего шага • ${rub(potential)}`,kind:"next"});else if(d.updatedAt&&Date.now()-Date.parse(d.updatedAt)>14*86400000)items.push({score:92+boost,dealId:d.id,title:d.name,meta:`Нет обновлений >14 дней • следующий шаг ${fmtDate(parseLocal(d.nextDate))}`,kind:"stale"});else if(q&&!q.ok)items.push({score:85+boost,dealId:d.id,title:d.name,meta:`Карточка ≥500k ${q.done}/${q.total} • следующий шаг ${fmtDate(parseLocal(d.nextDate))}`,kind:"quality"});else if(String(d.closeDate||"").startsWith(localMonthKey()))items.push({score:70+boost+(+d.probability||0)/10,dealId:d.id,title:d.name,meta:`${d.nextStep} • ${fmtDate(parseLocal(d.nextDate))} • ${d.probability||0}%`,kind:"close"})}return items.sort((a,b)=>b.score-a.score).slice(0,8)}
