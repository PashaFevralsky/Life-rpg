...
function dataIntegrityIssues(){
  ...
  const collections=[["incomeLogs",S.incomeLogs],["expenses",S.expenses],["payments",S.payments],["workLogs",S.workLogs],["tennis",S.tennis],["readingLogs",S.readingLogs],["crmDeals",S.crmDeals]];for(const [name,arr] of collections)for(const x of arr||[]){if(!x?.id)continue;const key=`${name}:${x.id}`;if(ids.has(key))add("bad",`Дублирующийся ID в ${name}`,x.id);ids.set(key,true)}
...
function renderSystemDiagnostics(){const box=$("systemDiagnostics");if(!box)return;const issues=dataIntegrityIssues();if(!issues.length){box.innerHTML='<div class="status"><b>OK.</b> Критичных проблем целостности данных не обнаружено.</div>';return}box.innerHTML=issues.map(x=>`<div class="notice ${x.level==="bad"?"diagnostic-bad":""}" style="margin-top:8px"><b>${escapeHtml(x.title)}</b>${x.detail?`<div class="sub">${escapeHtml(x.detail)}</div>`:""}</div>`).join("")}
