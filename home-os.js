...
async function homeBought(id){const x=personalData().shopping.find(q=>q.id===id);if(!x)return;x.done=true;x.doneAt=personalNow();await save(x.inventoryId?"Покупка отмечена — обнови фактический остаток":"Покупка отмечена")}
