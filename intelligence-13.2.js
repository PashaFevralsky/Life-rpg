...
/* Life RPG 13.2 — Decision Intelligence Kernel.
...
function intelligence132LearnedAdjustment(x){
  const sig=intelligence132Signature(x),rows=intelligence132Store().journal.filter(j=>j.signature===sig&&j.outcome);if(rows.length<3)return {adjustment:0,n:rows.length,rate:null};
  const vals=rows.slice(0,30).map(j=>j.outcome==="helped"?1:j.outcome==="done"?.35:j.outcome==="no-effect"?-1:j.outcome==="missed"?-.5:0),avg=vals.reduce((a,b)=>a+b,0)/vals.length,adj=Math.round(clamp(avg*10,-8,8));return {adjustment:adj,n:vals.length,rate:avg}
}
...
function intelligence132JournalExposure(x,rank){
  ...
  const row={id:uid(),type:"exposure",at:intelligence132Now(),dateKey:today,candidateId:id,signature:intelligence132Signature(x),area:x.area,kind:x.kind,title:x.title,rank,score:+x.score||0,confidenceScore:+x.confidenceScore||intelligence132ConfidenceScore(x),hard:!!x.hard,minutes:+x.minutes||0,taskId:x.taskId||"",projectId:x.projectId||"",evidence:(x.evidence||[]).slice(0,5),outcome:"",outcomeAt:""};st.journal.unshift(row);st.journal=st.journal.slice(0,INTEL132_MAX_JOURNAL);return row
}
