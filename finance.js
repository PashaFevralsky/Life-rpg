...
async function syncBalance(){const i=+$("syncDebt").value,b=+$("syncBalanceInput").value;if(!Number.isFinite(b)||b<0){toast("Укажи корректный остаток");return}const d=S.debts[i];if(!d){toast("Долг не найден");return}const now=new Date().toISOString();d.balance=b;d.balanceVerifiedAt=now;S.balanceHistory.push({id:uid(),date:localDateKey(),ts:now,total:totalDebt(),type:"sync",debtIndex:i,debtId:d.id,balance:b});$("syncBalanceInput").value="";closeModal("syncModal");await save("Остаток синхронизирован")}
