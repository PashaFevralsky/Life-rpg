...
function peopleHealth(person){const last=peopleLastInteraction(person.id),cad=Math.max(1,+person.cadenceDays||30);if(!last)return {last:null,days:null,due:false,score:null,needsBaseline:true};const days=personalDaysSince(last.occurredAt||last.dateKey),ratio=days==null?0:days/cad,score=Math.round(clamp(100-ratio*60,0,100));return {last,days,due:days!=null&&days>=cad,score,needsBaseline:false}}
